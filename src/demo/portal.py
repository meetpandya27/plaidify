"""Bundled demo portal — a self-contained target site for the Plaidify sandbox.

This is a small FastAPI app that imitates a real authenticated web portal: a
login form, an optional MFA challenge, and a data-rich dashboard. The
``demo_utility`` connector drives this site, so the full Plaidify loop
(login → MFA → structured extraction) can be exercised end-to-end without any
real external site.

Run it standalone::

    python -m src.demo.portal              # serves on http://127.0.0.1:8080
    DEMO_PORTAL_PORT=9000 python -m src.demo.portal

The element IDs and CSS classes below are the contract the ``demo_utility`` (and
the test ``internal_bank``) connector blueprints extract against — keep them in
sync if you change the markup.

Demo credentials:
    demo_user / demo_pass   → no MFA
    demo_mfa  / demo_pass   → MFA required (code: 123456)
"""
...
# Accounts recognised by the demo portal. The ``test_user`` / ``fixture_mfa``
# entries preserve backwards compatibility with the Playwright integration
# fixtures; ``demo_*`` are friendlier aliases used by the sandbox runner & docs.
VALID_USERS = {
    "test_user": {"password": "test_pass", "requires_mfa": False},
    "fixture_mfa": {"password": "test_pass", "requires_mfa": True},
    "demo_user": {"password": "demo_pass", "requires_mfa": False},
    "demo_mfa": {"password": "demo_pass", "requires_mfa": True},
}
MFA_CODE = "123456"
...
<style>
  ...
  .badge { background: #0b8f73; color: white; border-radius: 999px; font-size: 11px; padding: 2px 10px; margin-left: 8px; text-transform: uppercase; letter-spacing: 0.05em; }
  .error { color: #c2410c; min-height: 20px; }
  .hint { color: #5a6576; font-size: 13px; margin-top: 16px; }
</style>
"""
...
<body>
  <div class='topbar'>{BRAND}<span class='badge'>Sandbox</span></div>
  <div class='wrap'>
    <div class='card' style='max-width: 420px; margin: 80px auto;'>
      ...
      <p class='hint'>Demo logins: <code>demo_user</code> / <code>demo_pass</code> (no MFA)
        &middot; <code>demo_mfa</code> / <code>demo_pass</code> (MFA code <code>123456</code>)</p>
    </div>
  </div>
</body></html>"""
...
def _mfa_html(error: str = "") -> str:
    return f"""<!DOCTYPE html>
<html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1' /><title>{BRAND} Verification</title>{PAGE_STYLE}</head>
<body>
  <div class='topbar'>{BRAND}<span class='badge'>Sandbox</span></div>
  <div class='wrap'>
    <div class='card' style='max-width: 420px; margin: 80px auto;'>
      ...
      <p class='hint'>Demo verification code: <code>123456</code></p>
    </div>
  </div>
</body></html>"""
...
def _dashboard_html(username: str) -> str:
    return f"""<!DOCTYPE html>
<html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1' /><title>{BRAND} Dashboard</title>{PAGE_STYLE}</head>
<body>
  <div class='topbar'>{BRAND}<span class='badge'>Sandbox</span><a href='/logout' id='logout-btn' style='color:white; float:right;'>Sign Out</a></div>
  <div class='wrap'>
    ...
    <div class='card'>
      ...
      <div style='margin-top: 18px;'>
        ...
        <div id='customer-email'>alex.johnson@email.com</div>
      </div>
    </div>
    <div class='card'>
      <h2>Usage History</h2>
...
def create_app() -> FastAPI:
    """Build the demo portal FastAPI application."""
    app = FastAPI(title=f"Plaidify Demo Portal — {BRAND}")
    app.add_middleware(SessionMiddleware, secret_key="plaidify-demo-portal")
    ...
    async def dashboard(request: Request):
        if not request.session.get("authenticated"):
            return RedirectResponse("/login", status_code=303)
        return HTMLResponse(_dashboard_html(request.session.get("username", "demo_user")))
...
def main() -> None:
    """Run the demo portal with uvicorn (CLI entrypoint)."""
    import uvicorn

    host = os.environ.get("DEMO_PORTAL_HOST", "127.0.0.1")
    port = int(os.environ.get("DEMO_PORTAL_PORT", "8799"))
